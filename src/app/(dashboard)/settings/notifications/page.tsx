...
export default async function NotificationSettingsPage({
  ...
  const mockSmsOn = isMockSmsEnabled()
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">SMS de teste</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p className="text-muted-foreground text-sm">
            {mockSmsOn
              ? 'Simulação ativa: cada notificação com telefone também grava na caixa de SMS (custo R$ 0), mesmo quando o e-mail envia.'
              : 'Simulação desligada. Ative MOCK_SMS_ENABLED=true no ambiente para gravar SMS de teste.'}
          </p>
          <Link
